...
if($data == "allProducts"){
	...
	$sql = "SELECT p.id_product, p.name, p.price, p.low_price,p.part_number, p.max_sale, p.stock, p.pounds, p.ounces, p.length, p.width, p.height, p.id_cto_product_category, p.id_brand, p.photos, p.short_description FROM innopbbl_innopartserv.product as p";
...
if($data == "categoria"){
	$sql = "SELECT * FROM innopbbl_innopartserv.cto_product_category";
...
if($data == "brand"){
	$sql = "SELECT * FROM innopbbl_innopartserv.cto_brand";
...
if($data == "productStock"){
	...
	$sql = "SELECT stock as stock FROM innopbbl_innopartserv.product WHERE id_product = ".$_GET["id_product"].";";
