...
if($data == "allProducts"){
	...
	$sql = "SELECT p.id_product, p.name, p.price, p.low_price,p.part_number, p.max_sale, p.stock, p.pounds, p.ounces, p.length, p.width, p.height, p.id_cto_product_category, p.id_cto_brand, p.photos, p.short_description FROM innopbbl_tienda.product as p";
	...
	if ($result->num_rows > 0) {
	    ...
	    while($row = $result->fetch_assoc()) {
	        ...
	        $result_json = $result_json."{\"id_product\":\"".$row["id_product"]."\",\"name\":\"".$row["name"]."\",\"price\":\"".$row["price"]."\",\"low_price\":\"".$row["low_price"]."\", \"max_sale\":\"".$row["max_sale"]."\",\"stock\":\"".$row["stock"]."\",\"part_number\":\"".$row["part_number"]."\",\"photos\":".$row["photos"].",\"pounds\":\"".$row["pounds"]."\",\"ounces\":\"".$row["ounces"]."\",\"length\":\"".$row["length"]."\",\"width\":\"".$row["width"]."\",\"height\":\"".$row["height"]."\",\"id_cto_brand\":".$row["id_cto_brand"].", \"id_cto_product_category\":".$row["id_cto_product_category"].",\"short_description\":\"".addslashes($row["short_description"])."\"},";
	    }
...
if($data == "brand"){
	...
	if ($result->num_rows > 0) {
	    // output data of each row
	    while($row = $result->fetch_assoc()) {
	        $result_json = $result_json."{\"id_cto_brand\":\"".$row["id_brand"]."\",\"description\":\"".$row["description"]."\",\"photos\":".$row["photos"]."},";
	    }
	    $result_json = substr($result_json, 0, -1)."]";
	    echo $result_json;
	} else {
